...
def normalizar_nome(nome):
    ...
    # Remover tudo que não for letra, exceto espaços e parênteses que são removidos no re.sub
...
def calcular_vitorias(resultado):
    """Retorna 1 se for vitória, 0 caso contrário."""
    return 1 if resultado == 'V' else 0


def calcular_empates(resultado):
    """Retorna 1 se for empate, 0 caso contrário."""
    return 1 if resultado == 'E' else 0


def calcular_derrotas(resultado):
    """Retorna 1 se for derrota, 0 caso contrário."""
    return 1 if resultado == 'D' else 0


# --- Função Principal de Análise (Classificação por Rodada) ---
...
def analisar_classificacao_por_rodada(file_path, times_alvo_exibicao):
    try:
        ...
        # Criar listas de nomes normalizados e mapa de exibição
        ...
        # Processar jogos para cada time alvo
        for time_normalizado in times_alvo_normalizado:
            time_exibicao = mapa_exibicao.get(time_normalizado)

            # 1. Jogos como 'Time_Jogo' (Mandante ou Visitante, mas na coluna 'Time')
        ...
        # Garantir que cada jogo seja contado apenas uma vez por time alvo
        ...
        # Ordenar por Rodada e Time para garantir a acumulação correta
        df_final_jogos.sort_values(by=['Rodada', 'Time_Alvo'], inplace=True)

        # Agrupar por Rodada e Time_Alvo para somar as estatísticas do jogo
        ...
        # Garantir que todas as rodadas (1 a 38) estejam presentes para todos os times
        ...
        # Reordenar por Rodada e Time para garantir a acumulação correta
        df_stats_rodada.sort_values(by=['Time_Alvo', 'Rodada'], inplace=True)

        # Calcular estatísticas acumuladas
        ...
        # Criar a tabela de classificação por rodada
        ...
        for rodada in range(1, max_rodada + 1):
            ...
            # Critérios de desempate (Regra oficial do Brasileirão)
            # 1. Pontos
            # 2. Número de Vitórias
            # 3. Saldo de Gols
            # 4. Gols Pró
            # 5. Confronto Direto (Não implementado aqui, pois exigiria mais dados e complexidade)
            # 6. Menor número de Cartões Vermelhos (Não implementado)
            # 7. Menor número de Cartões Amarelos (Não implementado)

            # Usaremos os 4 primeiros critérios principais para a classificação
        ...
        # Pivotar a tabela para ter Rodada como índice e Times como colunas de Posição
        ...
        # Reordenar as colunas na ordem do G6 final
...
if __name__ == "__main__":
    # O caminho do arquivo CSV deve ser ajustado para o ambiente do sandbox
    # ATENÇÃO: Ajuste o caminho do arquivo CSV para o local correto no seu computador.
    # Exemplo: file_path = r"C:\Caminho\Para\Seu\Arquivo\SerieA2023.csv"
    ...
    # Execução do script (classificação por rodada)
